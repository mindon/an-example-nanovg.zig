...
pub fn main() !void {
    ...
    _ = vg.createFontMem("sans", @embedFile("assets/fonts/Roboto-Regular.ttf"));
    const image_sample = vg.createImageMem(@embedFile("assets/a(t)m.png"), .{
    ...
    while (c.glfwWindowShouldClose(window) == c.GLFW_FALSE) {
        ...
        // vg.fillPaint(vg.imagePattern(0, 0, fb_width, fb_width, 0, image_sample, 1));
        vg.fillPaint(vg.imageBlur(fb_width, fb_width, image_sample, blur_biased, 0));
        vg.fill();
        vg.endFrame();
        // blur_y
...

        {
            ...
            vg.beginPath();
            vg.rect(x, y, 256, 256);
            vg.fillPaint(vg.imagePattern(x, y, 256, 256, 0, image_sample, 1));
            vg.fill();
